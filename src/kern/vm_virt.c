...
static void enable_feature_map() {
  // Enable IA32_EFER.NX and CR4.SMEP (both should be available on CPUs since ~2011)
  if (MMU_CHECK(MM_FEAT_NX)) {
    asm_wrmsr(IA32_EFER, asm_rdmsr(IA32_EFER) | (1 << 11));
  } else {
    PANIC(NULL, "NX (x86_64 Execute Disable Bit) is not supported on this machine!");
  }
  if(MMU_CHECK(MM_FEAT_SMEP)) {
    asm_write_cr4(asm_read_cr4() | (1 << 20)); 
  } else {
    PANIC(NULL, "SMEP (x86_64 Supervisor Mode Execution Protection) is not supported on this machine!");
  }

  // Enable CR4.SMAP and CR4.PCIDE (both of which are not required)
  // Furthermore, both are not required since PCID is somehow not 
  // supported on my AMD Ryzen 5500U, which I find completly ABSURD.
...
static uint64_t flags_to_pte(uintptr_t phys, int flags) {
  uint64_t raw_page = 0;
	
  if(!(flags & VM_PERM_READ)) {
    log("vm/virt (WARN): Non-Readable mappings not supported (flags: 0x%x)", flags); 
  }
...
uint64_t* virt2pte(vm_space_t* spc, uintptr_t virt) {
...
static void vm_load_space(vm_space_t* spc) {
...
void vm_init_virt() {
  ...
  for (uintptr_t p = 0; p < 0x80000000; p += 0x1000) {
    vm_virt_map(&kernel_space, p, VM_KERN_OFFSET + p, VM_PERM_READ | VM_PERM_WRITE | VM_PERM_EXEC | VM_PAGE_GLOBAL);
  }
