...
static void set_char(struct tty *t,
                     int x,
                     int y,
                     char c,
                     uint32_t fg,
...
void insert_char(struct tty *t, char ch) {
  ...
  if (t->tio.c_lflag & ICANON) {
    switch (ch) {
      case '\n':
        if (t->line_idx == LINE_BUF_MAX) goto cleanup;
        t->line_buf[t->line_idx++] = ch;
        if (t->tio.c_lflag & ECHO) tty_putc(t, ch);

        // Empty the line buffer into the input buffer
        for (size_t i = 0; i < t->line_idx; i++) {
          if (t->in_idx == IN_BUF_MAX) goto cleanup;
  ...
  if ((ch >= 0x20 && ch <= 0x7e) && t->tio.c_lflag & ECHO) tty_putc(t, ch);
...
int tty_ioctl(struct tty *t, int64_t req, void *argp) { return 0; }
