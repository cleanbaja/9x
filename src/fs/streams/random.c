...
void srand(uint32_t s) {
  ...
  for (ctr = 1; ctr < n; ctr++)
    status[ctr] =
        (1852734613 * (status[ctr - 1] ^ (status[ctr - 1] >> 30)) + ctr);
}
...
ssize_t random_read(struct vnode *bck, void *buf, off_t offset, size_t count) {
  // TODO: Find more sources of entropy, since /dev/random is supposed to be
  // more secure
  srand((uintptr_t)buf);
  if (offset != 0) srand(offset);
...
static ssize_t urandom_read(struct vnode *bck,
                            void *buf,
                            off_t offset,
                            size_t count) {
  if (count % 4) {
...
void setup_random_streams() {
  uint32_t seed =
      ((uint32_t)0xadf8ca9f * (uint32_t)asm_rdtsc()) * ((uint32_t)0x923c6e2d) ^
