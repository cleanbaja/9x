...
tlsf_pool_t *tlsf_add_pool(tlsf_t *tlsf, void *mem, size_t bytes)
{
	...
	if (((ptrdiff_t)mem % ALIGN_SIZE) != 0) {
		kprint("tlsf_add_pool: Memory must be aligned by %u bytes.\n",
			(unsigned int)ALIGN_SIZE);
		return NULL;
	}

	if (pool_bytes < block_size_min || pool_bytes > block_size_max) {
		kprint("tlsf_add_pool: Memory size must be between %zu and %zu bytes.\n",
...
int test_ffs_fls()
{
	...
	if (rv) {
		kprint("test_ffs_fls: %x ffs/fls tests failed.\n", rv);
	}
	return rv;
}
...
{
	if (test_ffs_fls()) {
		kprint("ffs/fls test failed\n");
		return NULL;
	}

	if (((ptrdiff_t)mem % ALIGN_SIZE) != 0) {
		kprint("tlsf_create: Memory must be aligned to %u bytes.\n",
			(unsigned int)ALIGN_SIZE);
		return NULL;
	}
