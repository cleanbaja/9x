...
static bool strace_table_present() {
  if (!ksym_table) {
    return false;
  } else {
    return true;
...
static void find_kernel_slide() {
  struct stivale2_struct_tag_kernel_slide *kslide =
      stivale2_find_tag(STIVALE2_STRUCT_TAG_KERNEL_SLIDE_ID);
  if (kslide == NULL) {
    kernel_slide = 0;
  } else {
...
static char *addr_to_name(uintptr_t addr, uint64_t *offset) {
  ...
  if (kernel_slide == UINT64_MAX) find_kernel_slide();
  ...
  for (size_t i = 0;;) {
    ...
    i += 1;
    if (ksym_table[i].base == UINT64_MAX) break;
  }
...
uintptr_t strace_get_symbol(char *name) {
  if (name == NULL || !strace_table_present()) return 0x0;
...
void strace_unwind(uintptr_t *base) {
  ...
  for (;;) {
    ...
    if (name) klog_unlocked("  * 0x%lx <%s+0x%lx>\n", ret_addr, name, offset);
    else
      klog_unlocked("  * 0x%lx\n", ret_addr);
